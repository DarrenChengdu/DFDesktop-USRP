...
public:
    ...
    void SetUnitText(QString text) { units_label->setText(text); }
...
class SpacerEntry : public QWidget {
    Q_OBJECT

public:
    SpacerEntry(QWidget *parent = 0);
    ~SpacerEntry() {}

private:
    DISALLOW_COPY_AND_ASSIGN(SpacerEntry)
};
...
public:
    ...
    ~ComboEntry() {}
    int currentIndex() {return combo_box->currentIndex();}
...
class CheckBoxNumericEntry : public QWidget {
    Q_OBJECT

public:
    CheckBoxNumericEntry(const QString &label_text, double init_val,  const QString &units_text, QWidget *parent = 0);
    ~CheckBoxNumericEntry() {}

    double GetValue() {return line->GetValue();}
    bool IsChecked() const { return check_box->isChecked(); }
    void SetChecked(bool checked) { check_box->setChecked(checked); }
    void Enable(bool enable) {
        label->setEnabled(enable);
...
private:
    DISALLOW_COPY_AND_ASSIGN(CheckBoxNumericEntry)
};
...
class IPAddrEntry : public QWidget {
	Q_OBJECT

public:
	IPAddrEntry(const QString &label_text,
		QString ip,
		QWidget *parent = 0);
	~IPAddrEntry() {}
...
signals:
	void ipViewChanged(QString);

private:
	DISALLOW_COPY_AND_ASSIGN(IPAddrEntry)
};

//////////////////////////////////////////////////////////////////////////

class DirectoryEntry : public QWidget {
	Q_OBJECT

public:
	DirectoryEntry(const QString &label_text,
		QString dir,
		QWidget *parent = 0);
	~DirectoryEntry() {}

	void SetDir(const QString &dir) { directory = dir; }
...
private:
	QString directory; // local copy
	Label *label;
	QLineEdit *entry;
	SHPushButton *directory_btn;
...
class TripleNumeric : public QWidget {
	Q_OBJECT

public:
	TripleNumeric(const QString &label_text, double starting_value,
	...
	double GetValue(int idx) const { 
		...
		{
		case 0:
			value = entry_start->GetValue();
			break;
		case 1:
			value = entry_stop->GetValue();
			break;
		case 2:
			value = entry_step->GetValue();
			break;
		}
	...
	void SetValue(double v1, double v2, double v3) { 
		...
		stop = v2;
		step = v3;
		entry_start->SetValue(v1);
		entry_stop->SetValue(v2);
		entry_step->SetValue(v3);
	}
...
private:
	...
	Label *label;
	LineEntry *entry_start;
	LineEntry *entry_stop;
	LineEntry *entry_step;
	Label *units_label;
...
signals:
	void valueChanged(double v1, double v2, double v3);

private:
	DISALLOW_COPY_AND_ASSIGN(TripleNumeric)
};
