...
SpacerEntry::SpacerEntry(QWidget *parent) :
    QWidget(parent)
{
    move(0, 0);
    resize(ENTRY_WIDTH, ENTRY_H);
...
void CheckBoxEntry::resizeEvent(QResizeEvent *)
{
    ...
    label->resize(lblWidth - ENTRY_OFFSET, ENTRY_H);
    check_box->move(lblWidth + btnWidth/2, 0);
    check_box->resize(width() - lblWidth, ENTRY_H);

//    check_box->resize(width(), ENTRY_H);
}

CheckBoxNumericEntry::CheckBoxNumericEntry(const QString &label_text, double init_val,  const QString &units_text, QWidget *parent)
    : QWidget(parent)
{
    move(0, 0);
    resize(ENTRY_WIDTH, ENTRY_H);
    ...
    line = new LineEntry(VALUE_ENTRY, this);
    connect(line, SIGNAL(entryUpdated()), this, SLOT(entryChanged()));
    line->move(0, 0);
    line->SetValue(init_val);
...
void CheckBoxNumericEntry::resizeEvent(QResizeEvent *)
{
    int entryWidth = (width() < 200) ? 120 : qMin(width() - 80, 210);
    int lblWidth = qMax(200, width()) - entryWidth;

    label->resize(lblWidth - ENTRY_OFFSET, ENTRY_H);
    check_box->move(lblWidth, 0);
    int cbWidth = ENTRY_H;
    check_box->resize(cbWidth, ENTRY_H);

    line->move(lblWidth + cbWidth, 0);
    line->resize(qMax(200, width()) - lblWidth - cbWidth - units_label->width(), ENTRY_H);

    // Units label does not resize just moves
    units_label->move(width() - units_label->width(), 0);
}

void CheckBoxNumericEntry::entryChanged() {
    value = line->GetValue();
    emit valueChanged(value);
}

void CheckBoxNumericEntry::slotClicked(bool sta) {
    if (sta) {
        line->setEnabled(false);
    }
    else {
...
// IP entry widget
IPAddrEntry::IPAddrEntry(const QString &label_text,
	QString ip,
	QWidget *parent)
	: QWidget(parent),
	IP(ip)
{
	...
	//entry->setInputMask("000.000.000.000;");

	connect(entry, SIGNAL(editingFinished()), this, SLOT(editUpdated()));
}

void IPAddrEntry::SetIP(const QString &ip)
{
	if (ip == IP) {
		return;
	}
...
void IPAddrEntry::resizeEvent(QResizeEvent *)
{
	int entryWidth = (width() < 200) ? 120 : qMin(width() - 80, 210);
	int lblWidth = qMax(200, width()) - entryWidth;
...
void IPAddrEntry::editUpdated()
{
	IP = entry->text();
	emit ipViewChanged(IP);
}

//////////////////////////////////////////////////////////////////////////

DirectoryEntry::DirectoryEntry(const QString &label_text,
	QString dir,
	QWidget *parent)
	: QWidget(parent),
	directory(dir)
{
	...
	directory_btn = new SHPushButton("...", this);

	entry->setText(directory);

	connect(directory_btn, SIGNAL(clicked()), this, SLOT(buttonClicked()));
}

void DirectoryEntry::resizeEvent(QResizeEvent *)
{
	int restWidth = (width() < 200) ? 120 : qMin(width() - 80, 210);
	int lblWidth = qMax(200, width()) - restWidth;

	label->move(ENTRY_OFFSET, 0);
	label->resize(lblWidth - ENTRY_OFFSET, ENTRY_H);
	entry->move(lblWidth, 0);
	entry->resize(restWidth * 0.8, ENTRY_H);
	directory_btn->move(lblWidth + restWidth * 0.8, 0);
	directory_btn->resize(restWidth * 0.2, ENTRY_H);
}

void DirectoryEntry::buttonClicked()
{
...
//////////////////////////////////////////////////////////////////////////
TripleNumeric::TripleNumeric(const QString &label_text, double starting_value,
	double stopping_value, double stepping_value,
	const QString &units_text, QWidget *parent)
	: QWidget(parent), start(starting_value), stop(stopping_value), step(stepping_value)
{
	move(0, 0);
	resize(ENTRY_WIDTH, ENTRY_H);

	label = new Label(label_text, this);
	entry_start = new LineEntry(VALUE_ENTRY, this);
	entry_stop = new LineEntry(VALUE_ENTRY, this);
	entry_step = new LineEntry(VALUE_ENTRY, this);

	SetValue(starting_value, stopping_value, stepping_value);
	...
	connect(entry_start, SIGNAL(entryUpdated()), this, SLOT(entryChanged()));
	connect(entry_stop, SIGNAL(entryUpdated()), this, SLOT(entryChanged()));
	connect(entry_step, SIGNAL(entryUpdated()), this, SLOT(entryChanged()));
}

void TripleNumeric::resizeEvent(QResizeEvent *)
{
	int restWidth = (width() < 200) ? 120 : qMin(width() - 80, 210);
	int lblWidth = qMax(200, width()) - restWidth;
	...
	restWidth = restWidth - units_label->width();
	entry_start->move(lblWidth, 0);
	entry_start->resize(restWidth / 3, ENTRY_H);
	entry_stop->move(lblWidth + restWidth / 3, 0);
	entry_stop->resize(restWidth / 3, ENTRY_H);
	entry_step->move(lblWidth + restWidth * 2 / 3, 0);
	entry_step->resize(restWidth / 3, ENTRY_H);	
}

void TripleNumeric::entryChanged()
{
	double v1 = GetValue(0);
	double v2 = GetValue(1);
	double v3 = GetValue(2);

	emit valueChanged(v1, v2, v3);
}
